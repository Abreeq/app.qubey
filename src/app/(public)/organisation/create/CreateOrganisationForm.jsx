...
export default function CreateOrganisationForm() {
  ...
  const conditions = {
    name: [
      { required: true, msg: "Please enter your organization name" },
      { length: 3, msg: "Organization name must be at least 3 characters" },
    ],
    ...
    Object.entries(data).forEach(([key, value]) => {
      conditions[key]?.some((condition) => {
        if (
          condition.required &&
          (!value || value.trim() === "") &&
          !(key === "industryOther" && data.industry !== "Other")
        ) {
  ...
  const submit = async (e) => {
    ...
    const formObj = {
      ...form,
      industryOther: form.industry === "Other" ? form.industryOther : null,
    };
    ...
    // ✅ persist active org in DB
    await fetch("/api/organisation/switch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ organizationId: data.org.id }),
    });

    // ✅ instant UI update
    toast.success("Organisation created successfully 🎉");
    await update(); // 🔥 no args → forces full refetch from jwt callback
      ...
      <div className="custom-container">
        <div className="flex flex-col space-y-2 items-center mb-6 sm:mb-7">
          ...
          <p className="text-center text-base sm:text-lg max-w-2xl">
            Fill in your company details to configure your compliance dashboard
            tailored to your region and industry.
          </p>
        </div>

        {/* Form */}
        <div
          className={`custom-container relative z-10 max-w-4xl bg-white/50 backdrop-blur-2xl rounded-3xl shadow-[0_20px_40px_rgba(118,27,230,0.12)] border border-gray-200 p-4 sm:p-6`}
        >
          {/* Error message */}
          {authError && (
          ...
          <form onSubmit={submit} className="space-y-3 sm:space-y-5">
            ...
            <div>
              ...
              />
              {errorMsg.name && (
                <p className="text-red-600 text-sm mt-1 -mb-3 ml-2">
                  {errorMsg.name}
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onFocus={() =>
                    setErrorMsg((prev) => ({ ...prev, industry: "" }))
                  }
                  onChange={(e) => {
                ...
                {isOtherIndustry && (
                  <input
                    ...
                    onFocus={() =>
                      setErrorMsg((prev) => ({ ...prev, industryOther: "" }))
                    }
                    onChange={(e) =>
                      handleChange("industryOther", e.target.value)
                    }
                    className={`w-full px-3 sm:px-4 py-1.5 sm:py-2 mt-2 text-sm sm:text-base rounded-4xl bg-slate-100/80 border ${
                      errorMsg.industryOther
                        ? "border-red-500"
                        : "border-gray-300"
              ...
              <div>
                ...
                <select
                  ...
                  onFocus={() =>
                    setErrorMsg((prev) => ({ ...prev, companySize: "" }))
                  }
                  onChange={(e) => handleChange(e.target.name, e.target.value)}
        ...
        <p className="text-center text-sm mt-6">
          ...
          <a
            href="https://staging.qubey.ae/terms"
          ...
          and{" "}
          <a
            href="https://staging.qubey.ae/privacy"
            rel="noopener noreferrer"
            target="_blank"
            className="underline hover:text-purple-600 cursor-pointer transition-colors"
          >
